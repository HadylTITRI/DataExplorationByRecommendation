...
def fetch_mongodb_schema():
    ...
    for collection_name in collections:
        ...
        schema[collection_name] = collection.find_one()
    
    schema_df = pd.DataFrame.from_dict(schema, orient='index').transpose()
    return schema_df.to_dict()

# Layout for MongoDB schema display
mongodb_layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            html.H1("MongoDB Schema Viewer", className="text-center my-4"),
            html.Div(id='mongodb-schema')
        ])
    ])
])

@app.callback(
    Output('mongodb-schema', 'children'),
    Input('schema-button', 'n_clicks'),
    prevent_initial_call=True
)
def display_mongodb_schema(n_clicks):
    if n_clicks:
        schema = fetch_mongodb_schema()
        return dbc.Table.from_dataframe(pd.DataFrame(schema), striped=True, bordered=True, hover=True)
...
app.layout = html.Div([
    dbc.NavbarSimple(
        children=[
            dbc.Button("Dashboard", id="dashboard-button", color="primary", className="me-2"),
            dbc.Button("MongoDB Schema", id="schema-button", color="secondary"),
        ],
        brand="Data Exploration App",
        brand_href="#",
        color="dark",
        dark=True,
    ),
...
def display_page(dashboard_nclicks, schema_nclicks):
    ctx = dash.callback_context

    if not ctx.triggered:
        return dbc.Container("Welcome to the Data Exploration App", className="p-5")
    
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]

    if button_id == 'dashboard-button':
    ...
    elif button_id == 'schema-button':
        return mongodb_layout
