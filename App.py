...
def run_notebook(notebook_path, parameters=None, progress_update_callback=None):
    ...
    try:
        ...
        if progress_update_callback:
            progress_update_callback(-1)  # Indicate error
        return str(e)
...
progress_colors = ["info", "info", "info"]
progress_messages = []

def run_notebook_thread(notebook, index, site=None):
    global progress_values, progress_colors, progress_messages
    if notebook == 'APIdataExtraction.ipynb':
        error = run_notebook(notebook, f"site = '{site}'", lambda value: update_progress(index, value))
    else:
        error = run_notebook(notebook, None, lambda value: update_progress(index, value))
    if error:
        progress_colors[index] = "danger"
        progress_messages.append(html.Div(f"Error during {notebook}: {error}", style={'color': 'red'}))
    else:
        progress_colors[index] = "success"
        progress_messages.append(html.Div(f"{notebook} complete.", style={'color': 'white'}))
...
def collect_data(n_clicks, selected_site_text):
    if n_clicks and selected_site_text:
        ...
        notebooks = [
            ('APIdataExtraction.ipynb', 0),
            ('DataPreprocessing.ipynb', 1),
            ('Clustering.ipynb', 2)
        ]

        def run_notebooks():
            for notebook, index in notebooks:
                run_notebook_thread(notebook, index, site if notebook == 'APIdataExtraction.ipynb' else None)

        thread = threading.Thread(target=run_notebooks)
        thread.start()

        while thread.is_alive():
            time.sleep(1)  # Wait for the thread to finish

        return progress_values[0], progress_colors[0], progress_values[1], progress_colors[1], progress_values[2], progress_colors[2], progress_messages
